...
void main(){
	...
	//calculate sobel
	for( int i = 0; i<9; i++){
		color = texture(srcTex, uv+offset[i]);
		sumX += color * kernel[i];
	...
	vec4 glowTex = texture(srcTex, uv);
	
	// sine wave on the blue channel
    glowTex += vec4(vec3(g), 1.0);//vec4(g, g*0.5, g*(1.0+sin(time)/2.0), 1.0);

	outputcolor = vec4(vec3(g*5.0), 1.0);
}